...
const Canvas: FC = memo(() => {
    ...
    const isPanningMobile = useSelector((state: RootState) => state.canvasReducer.isPanningMobile)
    ...
    function touchDown(e: React.TouchEvent<HTMLCanvasElement>): void {
        ...
        canvasCtx.beginPath()
        dispatch(setFigureStartX(centerX + (e.touches[0].pageX - width/2)/zoom.currentScale));
        dispatch(setFigureStartY(centerY + (e.touches[0].pageY - height/2)/zoom.currentScale));
    ...
    function dispatchDuringTouchEvent(e: React.TouchEvent<HTMLCanvasElement>): void {
        if(!isPanningMobile) {
            if(figureState.figureType == 'line') {
                ...
                dispatch(pushCash([
                    figureState.figureType, 
                    centerX + (e.changedTouches[0].pageX - width/2)/zoom.currentScale,
    ...
    function touchUp(e: React.TouchEvent<HTMLCanvasElement>): void {
        ...
        if(isPanningMobile) {
            ...
            setDifY(startY - e.changedTouches[0].pageY)
            setIsAltKeyWasDown(false)
            setIsAltKeyDownBeforeMouse(false)
            return
        }
    ...
    return ( 
        <div className='canvas-wrapper'>
            ...
            panning={{activationKeys: ["Alt"], velocityDisabled: true, disabled: isMobile ? !isPanningMobile : isDisabled}}
            disablePadding={true}
            wheel={{disabled: true}}
